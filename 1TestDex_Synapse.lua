...
function rightClickMenu(sObj)
	...
	currentRightClickMenu = CreateRightClickMenu(
		{"Cut","Copy","Paste Into","Duplicate","Delete","Group","Ungroup","Select Children","Teleport To","Decompile Script","Insert Object","View Script","Save Instance","Call Function","Call Remote"},
		"",
		false,
		function(option)
			if option == "Cut" then
			...
			elseif option == "Decompile Script" then
				if not Option.Modifiable then return end
				local insertedParts = {}
				local list = Selection:Get()
				for i = 1,#list do
					pcall(function()
						Synapse:WriteFile(list[i].Name .. ".lua", Synapse:Decompile(list[i]))
					end)
				end
...
function openScript(scrObj)
	...
	local scrSource = Synapse:Decompile(scrObj)
